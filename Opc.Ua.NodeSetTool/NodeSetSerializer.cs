...
{
    internal class NodeSetSerializer
    {
        ...
        public void SaveJson(string filePath)
        {
            ...
            Json.AbstractOpenAPISchema.SerializerSettings.DefaultValueHandling = DefaultValueHandling.Ignore;
            Json.AbstractOpenAPISchema.SerializerSettings.NullValueHandling = NullValueHandling.Ignore;
            Json.AbstractOpenAPISchema.SerializerSettings.Formatting = Newtonsoft.Json.Formatting.Indented;
        ...
        private void LoadWellKnownAliases()
        {
            ...
            {
                m_aliases[alias.Alias] = alias.NodeId.Format(m_context, false);
            }
        }
        ...
        public void SaveArchive(string filePath, int maxNodesPerFile)
        {
            ...
            using (var tarWriter = WriterFactory.Open(gzipStream, ArchiveType.Tar, CompressionType.None))
            {
                foreach (var file in files)
                {
                    ...
                    Json.AbstractOpenAPISchema.SerializerSettings.Formatting = Newtonsoft.Json.Formatting.None;
                    Json.AbstractOpenAPISchema.SerializerSettings.DefaultValueHandling = DefaultValueHandling.Ignore;
                    Json.AbstractOpenAPISchema.SerializerSettings.NullValueHandling = NullValueHandling.Ignore;
            ...
            if (input.RequiredModel != null)
            {
                ...
                foreach (var item in input.RequiredModel)
                {
                    output.RequiredModels.Add(new Json.ModelReference()
                    {
                        ...
                        XmlSchemaUri = input.XmlSchemaUri,
                        VarVersion = input.Version,
                        ModelVersion = input.ModelVersion,
                        PublicationDate = input.PublicationDateSpecified ? input.PublicationDate : null
                    });
                }
            }
            ...
            if (input.RequiredModels != null)
            {
                ...
                foreach (var item in input.RequiredModels)
                {
                    models.Add(new Xml.ModelTableEntry()
                    {
                        ModelUri = input.ModelUri,
                        XmlSchemaUri = input.XmlSchemaUri,
                        Version = input.VarVersion,
                        ModelVersion = input.ModelVersion,
                        PublicationDate = input.PublicationDate ?? DateTime.MinValue,
                        PublicationDateSpecified = input.PublicationDate != null
                    });
                }
            ...
            if (input.References != null)
            {
                ...
                foreach (var reference in input.References)
                {
                    if (output != null)
                    {
                        ...
                        if (referenceTypeId == Opc.Ua.ReferenceTypeIds.HasModellingRule)
                        {
                            output.ModellingRuleId = ToJsonNodeId(reference.Value);
                            continue;
                        }

                        if (referenceTypeId == Opc.Ua.ReferenceTypeIds.HasTypeDefinition)
                        {
                    ...
                    {
                        ReferenceTypeId = ToJsonNodeId(reference.ReferenceType),
        ...
        private void Update(Xml.UANode output, Json.UANode input)
        {
            ...
            if (!String.IsNullOrEmpty(input.ModellingRuleId))
            {
                ...
                {
                    ReferenceType = Opc.Ua.ReferenceTypeIds.HasModellingRule.ToString(),
                    IsForward = true,
            ...
            if (!String.IsNullOrEmpty(input.TypeId))
            {
                references.Add(new Xml.Reference()
                {
                    ReferenceType = Opc.Ua.ReferenceTypeIds.HasTypeDefinition.ToString(),
                    IsForward = true,
            ...
            if (input.References != null)
            {
                foreach (var reference in input.References)
                {
                    references.Add(new Xml.Reference()
                    {
                        ...
                        Value = ToXmlNodeId(input.NodeId)
                    });
                }
            }
        ...
        private Xml.UAVariableType ToXmlNode(Json.UAVariableType input)
        {
            ...
            Update(output, input);
            output.DataType = ToXmlNodeId(input.DataType);
        ...
        private Json.Variant ToJsonVariant(XmlElement input)
        {
            ...
            using var decoder = new XmlDecoder(input, m_context);
            TypeInfo typeInfo = null;
            var value = decoder.ReadVariantContents(out typeInfo);
            decoder.Close();

            // use a top level array so WriteVariant can be used. 
            // the later code extracts the first element from the array.
            var variant = new Variant(value, typeInfo);
            using var encoder = new JsonEncoder(m_context, JsonEncodingType.Compact, topLevelIsArray: true);
            encoder.WriteVariant(null, variant);
            var json = encoder.CloseAndReturnText();

            var item = JArray.Parse(json).Children<JObject>().FirstOrDefault();

            return new Json.Variant(
                (int)variant.TypeInfo.BuiltInType,
                item.GetValue("Value"),
                (value is Matrix m) ? new List<int>(m.Dimensions) : null);
        }
...
;
        ...
        private Xml.RolePermission[] ToXmlRolePermission(IList<Json.RolePermission> input)
        {
            ...
            foreach (var item in input)
            {
                output.Add(new Xml.RolePermission() { Permissions = (uint)item.Permissions, Value = ToXmlNodeId(item.RoleId) });
            }
            ...
            if (input.Fields != null)
            {
                foreach (var field in input.Fields)
                {
                    fields.Add(new Xml.DataTypeField()
                    {
                        Name = field.Name,
                        SymbolicName = field.SymbolicName,
                        DataType = ToXmlNodeId(field.DataType),
